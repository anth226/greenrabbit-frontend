<script>
	...
	$: {
		if ($promoAssetsStore) {
			...
			$promoAssetsStore.forEach((e) => {
				e.data.forEach((element) => {
					if (element.schema.schema_name === schemaName) {
...
<div class="wrapper">
	<div class="header">
		...
		<h1>Green Rabbit Exclusive Staking</h1>
	...
	<div id="Staking" class="tabcontent" class:selected={currentTab === 'Staking'}>
		{#if available}
			<div class="grid">
				{#each $promoAssetsStore as collection, i}
					{#each $promoAssetsStore[i].data.filter((asset) => !asset.isStaked && asset.isStakeable && asset.schema.schema_name === schemaName) as nftData}
						<div
							on:click={() => {
								if (selected + staked < maxStaked) {
		...
		{:else}
			...
			font-weight: bold;">You don’t have any Exclusive NFTs available for staking</span
				>
			</div>
		{/if}
	</div>
	...
	<div id="Staked" class="tabcontent" class:selected={currentTab === 'Staked'}>
		{#if staked}
			<div class="grid">
				{#each $promoAssetsStore as collection, i}
					{#each $promoAssetsStore[i].data.filter((asset) => asset.isStaked && asset.schema.schema_name === schemaName) as nftData}
						<div
							on:click={() => {
								nftData.isSelected = !nftData.isSelected;
							}}
		...
		{:else}
			...
			font-weight: bold;">You haven’t staked any Exclusive NFTs</span
				>
			</div>
		{/if}
	</div>
	{#if currentTab == 'Staking'}
		<PromoStakeButton disabled={selected == 0 || (currentTab != 'Staking' && staked === 3)} />
	{:else}
